...
public class ProjectionTests {
    ...
    *
    * 2. 임베디드 타입 프로젝션
    *   엔티티와 거의 비슷하게 사용되며 조회의 시작점이 될 수 없다. FROM 절에 사용불가
    *   임베디드 타입은 영속성 컨텍스트에서 관리되지 않는다
    *
    * 3. 스칼라 타입 프로젝션
    *   숫자,문자,날짜 같은 기본 데이터 타입니다
    *  스칼라 타입은 영속성 컨텍스트에서 관리되지 않는다.
    *
    * 4. NEW명령어를 활용한 프로젝션
    *   다양한 종류의 단순 값들을 DTO로 바로 조회하는 방식으로 NEW 패키지명 DTO명을 쓰명 해당 DTO로 바로 반환받을 수 있다.
    ...
    * */
    ...
    public void  TypedQuery를_이용한_스칼라_타입_프로젝션_테스트(){
        String jpql = "SELECT c.categoryNameList FROM category_section03 c";
